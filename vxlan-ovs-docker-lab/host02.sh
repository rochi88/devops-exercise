...
sudo ovs-docker add-port ovs-br0 eth1 docker3 --ipaddress=192.168.1.12/24 --gateway=192.168.1.1
sudo docker exec docker3 ip a

sudo ovs-docker add-port ovs-br1 eth1 docker4 --ipaddress=192.168.2.12/24 --gateway=192.168.2.1
sudo docker exec docker4 ip a
...
sudo ovs-vsctl add-port ovs-br0 vxlan0 -- set interface vxlan0 type=vxlan options:remote_ip=192.168.100.33 options:key=1000
sudo ovs-vsctl add-port ovs-br1 vxlan1 -- set interface vxlan1 type=vxlan options:remote_ip=192.168.100.33 options:key=2000
